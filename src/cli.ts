...
program
  ...
  .version(version)
  .option('-c, --channel <channel>', 'Slack channel ID or name', process.env.SLACK_CHANNEL)
  ...
  .option('--thread-timeout <seconds>', 'Thread inactivity timeout in seconds', '3600')
  .action(async (options) => {
    try {
      // Validate required options
      if (!options.channel) {
        throw new Error('Slack channel is required. Set SLACK_CHANNEL env var or use -c option.');
      }
      if (!options.token && !options.dryRun) {
        throw new Error('Slack token is required. Set SLACK_BOT_TOKEN env var or use -t option.');
      }
      ...
      const slackClient = options.dryRun ? null : await initializeSlackClient(options.token);
